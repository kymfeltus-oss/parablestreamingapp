...
print("🏠 Updating Home Dashboard (C-Dub Branding + Image Fix)...")

home_page_code = """import Navbar from "@/components/Navbar";
import HeroSection from "@/components/HeroSection";
import { creators } from "@/lib/preachers";
import Link from "next/link";
import { Activity, Users, DollarSign, Music, Mic2, Headphones, UploadCloud } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mt-6">
        ...
        <div className="mb-10 bg-[#111] border border-white/10 rounded-2xl p-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        {/* FIXED: Changed to .png to match your real file */}
                        <img 
                            src="/td_avatar.png" 
                            className="w-16 h-16 rounded-full border-2 border-violet-500 p-1 object-cover" 
                        />
            ...
            <div className="mt-6 pt-6 border-t border-white/5 flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                <button className="flex items-center gap-2 bg-white text-black px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wide hover:bg-gray-200 transition shrink-0">
                ...
                {/* REBRANDED C-DUB BUTTON */}
                <button className="flex items-center gap-2 bg-[#222] text-white px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wide hover:bg-[#333] transition border border-white/10 shrink-0">
                    <UploadCloud className="w-4 h-4 text-blue-400" /> Upload C-Dub Loop
                </button>

                {/* NEW BUTTON FOR MUSICIANS */}
                <button className="flex items-center gap-2 bg-[#222] text-white px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wide hover:bg-[#333] transition border border-white/10 shrink-0">
