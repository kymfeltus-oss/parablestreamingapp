      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className="lg:col-span-8 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatChip label="Seeds" value={profile?.seeds_balance || 0} onClick={() => go("/dashboard/coming-soon/seeds")} />
            <StatChip label="Followers" value={profile?.followers_count || 0} onClick={() => go("/dashboard/coming-soon/followers")} />
            <StatChip label="Following" value={profile?.following_count || 0} onClick={() => go("/dashboard/coming-soon/following")} />
            <StatChip label="Profile" value="Edit" onClick={() => go("/profile-setup")} />
          </div>
          ...
          <Panel title="Live Activity">
            {postsLoading ? (
              ...
              <div className="text-gray-400 text-sm">No activity available yet.</div>
            ) : (
...
function SpotlightCard({ title, headline, sub, primaryLabel, primaryPath, secondaryLabel, secondaryPath, go }: any) {
  return (
    <div className="rounded-2xl bg-zinc-900 p-6 relative overflow-hidden">
      ...
      <div className="relative flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-3">
          <button onClick={() => go(primaryPath)} className="bg-green-500 hover:bg-green-400 text-black px-5 py-3 rounded-xl font-semibold">{primaryLabel}</button>
          <button onClick={() => go(secondaryPath)} className="border border-green-500/50 text-green-400 px-5 py-3 rounded-xl font-semibold">{secondaryLabel}</button>
        </div>
      </div>
    </div>
  );
...
function HeaderBar({ avatarUrl, displayName, accountType, onEditProfile }: any) {
  return (
    <div className="rounded-2xl bg-zinc-900 p-5 flex items-center gap-4">
      <img src={avatarUrl} className="w-12 h-12 rounded-full object-cover" alt="Profile" />
      <div className="flex-1">
        <div className="text-xl font-bold">{displayName}</div>
        <div className="text-xs text-gray-400 capitalize">{accountType}</div>
      </div>
      <button onClick={onEditProfile} className="bg-green-500 hover:bg-green-400 text-black px-4 py-2 rounded-xl">Edit Profile</button>
    </div>
  );
}

function CommandStrip({ isCreator, onFeed, onGoLive, onUpload, onApplyCreator }: any) {
  return (
...
function Cmd({ label, onClick }: any) {
  return (
    <button onClick={onClick} className="px-4 py-2 rounded-xl bg-black/40 border border-green-500/25 hover:border-green-500/60 whitespace-nowrap">{label}</button>
  );
}
...
function StatChip({ label, value, onClick }: any) {
  return (
    <div onClick={onClick} className="cursor-pointer rounded-2xl bg-zinc-900 p-4 hover:bg-zinc-800 transition">
...
function ActivityRow({ avatarUrl, name, content, onClick }: any) {
  return (
    <div onClick={onClick} className="flex gap-3 p-3 rounded-xl bg-black/40 hover:bg-black/60 cursor-pointer transition">
      <img src={avatarUrl} className="w-10 h-10 rounded-full object-cover" alt="User" />
      <div className="flex-1 min-w-0">
        <div className="text-sm font-bold truncate">{name}</div>
        <div className="text-xs text-gray-400 line-clamp-1">{content}</div>
      </div>
    </div>
  );
}
